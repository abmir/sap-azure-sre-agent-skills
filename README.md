...
1. **Create the agent & enable capabilities.** At [sre.azure.com](https://sre.azure.com) create an SRE Agent and note its **Managed Identity** object ID (Identity blade).
   - **Tools & Skills:** In **Capabilities → Tools** and **Capabilities → Skills**, leave everything at the **defaults** — no changes needed. Your 13 SAP skills are **custom** (installed in step 5) and rely on the built-in **tools**, not the platform's built-in **skills**, so the default skill set is fine. *(Optional for SAP: enable `log_analytics_query` for ad-hoc AMS queries, and `azure_alerting_incident_handler` if you'll auto-trigger investigations from AMS alerts.)* Access is enforced by the agent's **Managed Identity + read-only RBAC** (step 2), so the tool toggles aren't the guardrail. Further reading: [Manage global tools](https://learn.microsoft.com/azure/sre-agent/manage-global-tools) · [Tools & skills page](https://learn.microsoft.com/azure/sre-agent/global-tools-page) · [Tools overview](https://learn.microsoft.com/azure/sre-agent/tools).
